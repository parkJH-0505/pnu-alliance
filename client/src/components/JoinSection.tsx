...
const INIT: FormData = {
  ...
  company:"",position:"",industry:"",motivation:"",
};
...
export default function JoinSection() {
  ...
  return (
    <section id="join" className="relative py-28 lg:py-40 overflow-hidden bg-charcoal-deep">
      <div className="absolute inset-0 noise-overlay" />
      <div className="absolute top-0 right-0 w-[500px] h-[500px] rounded-full [background:radial-gradient(circle,oklch(0.75_0.18_49.77_/_0.02),transparent)] blur-[120px] pointer-events-none" />
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8" ref={ref}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.1 }}>
            ...
            <div className="border border-gold/15 [background:oklch(0.2_0.06_40_/_0.5)] p-7 lg:p-9">
              <AnimatePresence mode="wait">
                ...
                {step === 3 && (
                  <motion.div key="s3" initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} transition={{duration:0.5}} className="py-8 text-center">
                    <motion.div initial={{scale:0}} animate={{scale:1}} transition={{type:"spring",stiffness:200,delay:0.2}} className="w-16 h-16 [background:oklch(0.75_0.18_49.77_/_0.1)] border border-gold/30 flex items-center justify-center mx-auto mb-6">
              ...
              {!submitted && (
                <div className="flex gap-3 mt-8">
                  {step > 1 && (
                    <button onClick={handleBack} disabled={submitting} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 border border-gold/25 text-ivory/70 text-sm tracking-[0.15em] uppercase hover:border-gold hover:text-gold transition-all duration-300 disabled:opacity-50">
                      <ArrowLeft size={14} />
                      이전
                    </button>
                  )}
                  <button onClick={handleNext} disabled={submitting} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-gold text-charcoal-deep text-sm tracking-[0.15em] uppercase hover:bg-gold-light transition-all duration-300 disabled:opacity-50">
                    {submitting ? (
                      <>
